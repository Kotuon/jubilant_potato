...
ABase_Projectile::ABase_Projectile() {
    ...
    if ( !collision_component ) {
        ...
        collision_component = CreateDefaultSubobject< USphereComponent >(
        ...
        collision_component->OnComponentHit.AddDynamic(
            this, &ABase_Projectile::OnHit );
    }

    if ( !projectile_movement_component ) {
        // Use this component to drive this projectile's movement.
        projectile_movement_component =
            CreateDefaultSubobject< UProjectileMovementComponent >(
                TEXT( "projectile_movement_component" ) );
        projectile_movement_component->SetUpdatedComponent(
            collision_component );
...
void ABase_Projectile::BeginPlay() {
    ...
    has_muzzle = false;
    // UGameplayStatics::SpawnSoundAttached(InAirSound, GetRootComponent(),
    // "None", FVector(0), FRotator(0)); UGameplayStatics::SpawnSoundAttached(
    // InAirSound, GetRootComponent(), "None", FVector( 0 ), FRotator( 0 ),
    // EAttachLocation::KeepRelativeOffset, false, 1.f, 1.f, InAirStartTime );
}
...
void ABase_Projectile::FireInDirection( const FVector& ShootDirection ) {
    projectile_movement_component->Velocity =
        ShootDirection * projectile_movement_component->InitialSpeed;
}

void ABase_Projectile::OnHit( UPrimitiveComponent* HitComponent,
                              AActor* OtherActor,
