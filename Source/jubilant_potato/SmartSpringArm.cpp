...
void USmartSpringArm::UpdateFromVelocity( float DeltaTime ) {
    ...
    if ( parent->GetVelocity().Length() > start_velocity ) {
        curr_time_velocity =
            FMath::Clamp( curr_time_velocity + DeltaTime, 0, lerp_time );
    } else {
        curr_time_velocity =
            FMath::Clamp( curr_time_velocity - DeltaTime, 0, lerp_time );
    }
...
bool USmartSpringArm::UpdateFromAiming( float DeltaTime ) {
    ...
    if ( !is_aiming ) {
        curr_time_aiming =
            FMath::Clamp( curr_time_aiming - DeltaTime, 0, lerp_time );
    } else {
        curr_time_aiming =
            FMath::Clamp( curr_time_aiming + DeltaTime, 0, lerp_time );
    }
...
void USmartSpringArm::TickComponent(
...
void USmartSpringArm::SetIsAiming( bool IsAiming ) { is_aiming = IsAiming; }
